...
_CONCRETE_COST = {
    "C20/25": 0.80, "C25/30": 0.90, "C30/37": 1.00,
    "C35/45": 1.12, "C40/50": 1.25, "C45/55": 1.40,
    "C50/60": 1.60,
}
...
def objective(h):
    ...
    cost = (unit_cost_concrete * vol_concrete * L      # concrete
            + COST_STEEL * A_s * 1e-3 * L)             # steel
...
if __name__ == "__main__":
    ...
    print(f"\nOptimal design:")
    print(f"  Concrete grade : {grade.name}  "
          f"(fck = {fc} MPa, Ecm = {grade.Ecm_GPa} GPa)")
    print(f"  Section        : b = {b:.0f} mm × d = {d:.0f} mm")
    print(f"  {rebar_var.describe(code)}  "
          f"(A_s = {A_s:.0f} mm²)")
    print(f"\nStrength check:")
    print(f"  φMn = {phi_Mn:.1f} kN·m  ≥  Mu = {Mu} kN·m  "
          f"{'✓' if phi_Mn >= Mu else '✗'}")
    print(f"\nPenalty : {result.best_penalty:.4f}  "
          f"({'feasible' if result.best_penalty <= 0 else 'INFEASIBLE'})")