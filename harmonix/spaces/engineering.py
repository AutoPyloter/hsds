...
def _bar_is_valid_single(
    ...
    return rho_min <= rho <= rho_max and c < c_max and spacing >= min_sp
...
class ACIRebar(_DynamicGridVariable):
    ...
    @property
    def fc(self):
        return self._fc if not callable(self._fc) else None

    @property
    def fy(self):
        return self._fy if not callable(self._fy) else None
...
class SteelSection(CatalogueVariable):
    ...
    def __init__(
        ...
        if not self._sections:
            raise ValueError(f"No sections found for series={series!r}. Available: {sorted({s.series for s in base})}")
...
class SeismicZoneTBDY(CatalogueVariable):
    ...
    def __init__(
        ...
        filtered = [
            z
            for z in _SEISMIC_ZONES
            if (hazard_levels is None or z.hazard_level in hazard_levels)
            and (site_classes is None or z.site_class in site_classes)
        ]
        if not filtered:
            raise ValueError(f"No seismic zones match hazard_levels={hazard_levels}, site_classes={site_classes}.")
    ...
    def describe(self, index: int) -> str:
        z = self.decode(index)
        return f"{z.name}  (SDS={z.SDS:.3f} g, SD1={z.SD1:.3f} g, PGA={z.PGA:.2f} g)"
