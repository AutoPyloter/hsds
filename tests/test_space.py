...
class TestNaturalNumber:
    ...
    def test_neighbor_at_bounds(self):
        v = NaturalNumber(hi=5)
        # At lower bound (1): only valid neighbor is 2
        for _ in range(20):
            nb = v.neighbor(1, {})
            assert nb in {1, 2}, f"neighbor of 1 should be 1 or 2, got {nb}"
        # At upper bound (5): only valid neighbor is 4
        for _ in range(20):
            nb = v.neighbor(5, {})
            assert nb in {4, 5}, f"neighbor of 5 should be 4 or 5, got {nb}"
