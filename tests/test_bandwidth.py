...
class TestContinuousNeighborBw:
    def test_large_bw_produces_larger_steps(self):
        ...
        random.seed(0)
        steps_large = [abs(v.neighbor(50.0, {"__bw__": 0.50}) - 50.0) for _ in range(500)]
        random.seed(0)
        steps_small = [abs(v.neighbor(50.0, {"__bw__": 0.01}) - 50.0) for _ in range(500)]
...
class TestPitchAdjustmentIntegration:
    def test_par_zero_never_calls_neighbor(self):
        ...
        Minimization(space, lambda h: (h["x"] ** 2, 0.0)).optimize(memory_size=5, max_iter=100, par=0.0)
    ...
    def test_par_one_always_calls_neighbor_on_memory_hit(self):
        ...
        space = DesignSpace()
        space.add("x", CountingVar(0.0, 1.0))
        Minimization(space, lambda h: (h["x"] ** 2, 0.0)).optimize(memory_size=5, max_iter=50, hmcr=1.0, par=1.0)
    ...
    def test_bw_decay_improves_convergence(self):
        ...
        random.seed(42)
        r_decay = Minimization(space, sphere).optimize(memory_size=15, max_iter=1000, bw_max=0.3, bw_min=0.001)
        random.seed(42)
        r_const = Minimization(space, sphere).optimize(memory_size=15, max_iter=1000, bw_max=0.3, bw_min=0.3)
